...
//                echo '<script>alert("Date Condition Matched!");</script>';
    ...
    if(mysqli_num_rows($res)==0){
        /*        echo '<script>
                    alert("No Scheduels Set for the device!!");
                </script> ';*/
        /*
...
//        echo '<script>alert("'.$email.'");</script>';
        ...
        $sql = "UPDATE  user_and_devices SET server_request='$action' WHERE (user_id=$id AND machine_mac='$machine_mac')";
        mysqli_query($con, $sql);
        ...
        if($confirm_msg!="Record Saved"){
            ...
            //     $action = $_GET["action"];
            //     $row_id = $_GET["row_id"];
            $redirect_to = "dashboard.php?row_id=$row_id&action=$action&machine_mac=$machine_mac";
        ...
        if($confirm_msg=="Record Saved"){
            send_notification($con, "Action Set to $action");
            echo '<script>
//            alert("Action Set to '.$action.'. Message from server: '.$confirm_msg.'");
